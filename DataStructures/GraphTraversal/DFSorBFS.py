...
def breadth_first_search(problem):
    ...
    while frontier:
        ...
        if problem.goal_test(state):
            return [problem.initial_state] + path
        ...
        for action in problem.actions(state):
            ...
            if child_state not in explored and all(child_state != n[0] for n in frontier):
                frontier.append((child_state, child_path, cost + 1))
...
if __name__ == "__main__":
    ...
    choice = input("Choose search method (BFS/DFS): ").strip().upper()

    if choice == "BFS":
        solution_path = breadth_first_search(problem_instance)
    elif choice == "DFS":
        solution_path = depth_first_search(problem_instance)
    else:
        print("Invalid choice!")
        solution_path = None

    if solution_path:
        print(f"{choice} Solution Path:", " → ".join(solution_path))
    else:
        print("No path found")
